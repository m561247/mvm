...
// synthStructForDerive builds a synth struct rtype to use as elem in the
// derived-type tests below.
// Mirrors the AttachStructMethods + StructOf pattern used elsewhere in the
// test suite.
func synthStructForDerive(t *testing.T, name string) reflect.Type {
	...
	rt, err := AttachStructMethods(
		...
		[]Method{{
			...
			Handler: stubHandler(new(bool), name),
		}},
	)
	if err != nil {
		t.Fatalf("AttachStructMethods(%q): %v", name, err)
	}
	return rt
}
...
// TestDerivedFmtStringer end-to-end: derived []T on a synth struct can be
// printed via fmt.Sprintf.
// The elem has a String() method (stub returns the synth type's name), so
// fmt formats each element via that stringer rather than walking fields.
func TestDerivedFmtStringer(t *testing.T) {
	elem := synthStructForDerive(t, "DeriveFmt")
	sl := SliceOf(elem)
	v := reflect.MakeSlice(sl, 2, 2)
	v.Index(0).FieldByName("V").SetInt(1)
	v.Index(1).FieldByName("V").SetInt(2)
	s := fmt.Sprintf("%v", v.Interface())
	if !contains(s, "DeriveFmt") {
